...
export default function FreeShippingPage() {
  ...
  const [form, setForm] = useState({ enabled: false, threshold: 0 });

  useEffect(() => {
    fetch("/api/merchant/settings").then((r) => r.json()).then((d) => {
      const s = d.store?.settings ?? {};
      setForm({ enabled: s.freeShippingEnabled === true, threshold: Number(s.freeShippingThreshold ?? 0) });
  ...
  async function save(e: React.FormEvent) {
    ...
    try {
      const r = await fetch("/api/merchant/settings", {
        ...
        body: JSON.stringify({ name: "مسبار", design: { freeShippingEnabled: form.enabled, freeShippingThreshold: Number(form.threshold) } }),
      });
      const d = await r.json();
      if (!r.ok) return toast.error(d.error || "تعذر الحفظ");
      toast.success("تم حفظ إعدادات الشحن المجاني");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div><h2 className="text-2xl font-bold">الشحن المجاني</h2><p className="text-sm text-ink-300 mt-1">شجّع طلبات أكبر بتقديم شحن مجاني</p></div>
      {!loaded ? (
        ...
        <form onSubmit={save} className="glass rounded-3xl p-6 space-y-5">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1"><p className="font-bold text-sm">تفعيل الشحن المجاني</p><p className="text-[11px] text-ink-300">شحن مجاني للطلبات فوق الحد</p></div>
          ...
          <label className="block">
            <span className={lbl}>الحد الأدنى للطلب (ر.س)</span>
            <input type="number" className={inp} min={0} value={form.threshold} onChange={(e) => setForm({ ...form, threshold: Number(e.target.value) })} disabled={!form.enabled} />
          </label>
